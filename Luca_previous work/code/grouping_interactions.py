...
radii_distribution_by_category = 'radii_distribution.pkl'
...
def process_interactions_v2(df, thresholds,time_uncertainty_model, small_coeff,big_coeff, N_groups_to_make):
    """
    ...
    start_index = 0
    end_index = N_groups_to_make + 1000
    ...
    while len(ungrouped_interactions) > 0 and count < N_groups_to_make:
        ...
        print(len(window_interactions)+1)
        '''else:
...
for name in time_unc_models.columns[8:10]:
        ...
        for small_coeff in [3,4]:
            for big_coeff in [4]:
                for perc in [75]:
                    ...
                    grouped  = process_interactions_v2(df_unc,thresholds,unc_model,small_coeff,big_coeff,N_groups_to_make)
